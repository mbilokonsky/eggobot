...
const no_instructor_found = (user, bot) => {
  console.log('No instructor found by email or name. Notify admin.');  
   bot.say({
    channel: process.env.ADMIN_CHANNEL,
    text: `Hey so I was trying to add ${user} to our list of instructors but I didn't have any luck figuring out which instructor account they should be associated with.
    
    If you know, and you'd like to handle it, just type \`/associateInstructor [slack user] [egghead API slug]\` to do it manually.`
   });
}
...
module.exports = function (controller) {
  ...
  controller.on('interactive_message_callback', (bot, message) => {
    ...
    if (pendingCallbacks[callback_id]) {
      ...
      if (result === "yes") {
        bot.replyInteractive(message, "Ok awesome, this instructor will be wired up!.");
      } else {
        bot.replyInteractive(message, "Ok, we'll keep trying to find them, standby.'");
      }
