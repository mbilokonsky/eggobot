...
I couldn't find an email match so I'm doing a name match, which is less reliable.`,
    attachments:[
      {
          title: `Is ${user.name} the instructor /instructors/${instructor.slug}?`,
...
module.exports = function (controller) {
  ...
  controller.on('interactive_message_callback', (bot, message) => {
    ...
    if (pendingCallbacks[callback_id]) {
      ...
      console.dir(message);
      const originalText = message.originalMessage.text;

      if (result === "yes") {
        bot.replyInteractive(message, `${originalText} 
          - ${message.user.name} says yes!`);
      } else {
        bot.replyInteractive(message, `${message.user.name} says no!`);
      }
